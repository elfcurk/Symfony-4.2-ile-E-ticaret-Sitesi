...
class OrdersController extends AbstractController
{
    ...
    {   $user = $this->getUser();
        $userid = $user->getid();
        return $this->render('orders/index.html.twig', ['orders' => $ordersRepository->findAll(['userid'=> $userid])]);
    }
    ...
    public function new(Request $request, ShopcartRepository $shopcartRepository): Response
    {
        ...
        if($this->isCsrfTokenValid('form-order', $submittedToken)){
            if ($form->isSubmitted()) {
                //kredi kartı bilgilerini, gerekli banka servisine gönder
                ...
                $query->execute();
                $this->addFlash('success', 'Siparişiniz Başarıyla Gerçekleştirilmiştir..Bizi Tercih Ettiğiniz İçin Teşekkür Ederiz');
